...
function inscriptionEtudiant($id_user, $cin, $nom, $matricule, $prenom, $date_naissance, $adresse, $lieu_naissance, $telephone, $email)
{
    ...
    if (!empty($_POST["username"]) && !empty($_POST["cin"]) && !empty($_POST["type"]) && !empty($_POST["nom"]) && !empty($_POST["prenom"]) && !empty($_POST["email"]) && !empty($_POST["telephone"])) {
            ...
            if ($id_user != null) {
                if ($_POST["type"] == "etudiant") {
                    ...
                    if (inscriptionEtudiant($id_user, $_POST["cin"], $_POST["nom"], $_POST["matricule"], $_POST["prenom"], $_POST["date_naissance"], $_POST["adresse"], $_POST["lieu_naissance"], $_POST["telephone"], $_POST["email"])) {
...
function listEtudiants()
{
    ...
    $sttm = $db->prepare("SELECT e.id, e.nom, e.prenom, e.email, e.matricule, e.date_naissance, e.telephone, e.email FROM etudiant e");
    if ($sttm->execute()) {
...
function listAbsenceParEtudiant($id)
{
    ...
    if ($sttm->execute()) {
        ...
        foreach ($absences_ as $a) {
            $sttm2 = $db->prepare("SELECT * FROM professeur where id=:id");
            $sttm2->bindParam(":id", $a["professeur"]);
    ...
    if ($sttm->execute()) {
        ...
        foreach ($absences_ as $a) {
            ...
            if ($sttm2->execute()) {
                ...
                $a["matricule"] = $etudiant["matricule"];
...
function update_student($etd)
{
    ...
    $sttm = $db->prepare("UPDATE etudiant
        ...
        SET utilisateur.login = :login, utilisateur.password = :password, etudiant.nom = :nom,etudiant.prenom = :prenom, etudiant.cin = :cin, etudiant.matricule = :matricule, etudiant.email = :email, etudiant.date_naissance = :date_naissance, etudiant.adresse = :adresse, etudiant.lieu_naissance = :lieu_naissance, etudiant.telephone = :telephone WHERE etudiant.id = :id");

    $sttm->bindParam(':id', $etd['id']);
    $sttm->bindParam(':cin', $etd['cin']);
    $sttm->bindParam(':nom', $etd['nom']);
    $sttm->bindParam(':matricule', $etd['matricule']);
...
if (isset($_POST["modifier_etudiant"])) {
    ...
    if (!empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['telephone']) && !empty($_POST['cin']) && !empty($_POST['matricule']) && !empty($_POST['date_naissance']) && !empty($_POST['lieu_naissance']) && !empty($_POST['adresse']) && !empty($_POST['email']) && !empty($_POST['username']) && !empty($_POST['password'])) {


        $etd['id'] = $_POST['id'];
        $etd['cin'] = $_POST['cin'];
        $etd['nom'] = $_POST['nom'];
        $etd['matricule'] = $_POST['matricule'];
